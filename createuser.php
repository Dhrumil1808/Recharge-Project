...
include("dbrecharge.php");
	...
	$user="SELECT * From user_details Where Email='$_COOKIE[email]'";
	$u=mysql_query($user);
	$us=mysql_fetch_object($u);
	...
	$i="INSERT INTO user_details(user_ID, Name, Mobile_No, Email, Password,Balance,Dealer,DealerID,hash,active) VALUES ('NULL','$n','$m','$e','$p',0,0,'$us->user_ID','$hash',0)";
	mysql_query($i);
...
http://erecharge.futurevisioncomputers.com/index.php?email='.$email.'&hash='.$hash.'";
	...
	header("location:index.php?ver=Verification link has been send to the EmailID");
...
<div id="signup_form">
<form name="Signup"  onsubmit="return validate()" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">

<span class="top"> Username </span>
<input type="text" id="a1"name="name" placeholder="Username" onBlur="username()"/> 
<br />
<ul> <div id="j"> <li id="1"> Check Availability </li> </div> </ul>
<br />
<div id="a"> <?php if(isset($_POST['submit']))
			{
				$n=$_POST['name'];
...
<div id="b"> <?php if(isset($_POST['submit']))
			{
				$m=$_POST['mobile'];
...
<span class="top"> E-mail </span> 
 ...
 <div id="c"><?php if(isset($_POST['submit']))
			{
				$e=$_POST['email'];
